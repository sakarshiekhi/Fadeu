...
class SavedWordListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        # Return saved words for the current user, ordered by most recent first
        return SavedWord.objects.filter(user=self.request.user).select_related('word')
    ...
    def create(self, request, *args, **kwargs):
        ...
        if SavedWord.objects.filter(user=request.user, word=word).exists():
            return Response(
        ...
        # Save the word
        saved_word = SavedWord.objects.create(user=request.user, word=word)
