...
class _FlashcardState extends State<Flashcard> {
  ...
  Future<void> _toggleSave() async {
    ...
    try {
      ...
      final success = await apiService.toggleSaveWord(widget.word.id);
      
      if (!success) {
        throw Exception('Failed to update save status on server');
      }
      
      // Update local storage
      final prefs = await SharedPreferences.getInstance();
      
      // Update bookmarked word IDs
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Word ${newSaveState ? 'saved' : 'unsaved'} successfully'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to ${newSaveState ? 'save' : 'unsave'} word: ${e.toString()}'),
